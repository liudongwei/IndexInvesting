...
/**
 * 三根K线形态识别器
 * 包括：十字启明星、十字黄昏之星、启明星、黄昏之星、三只乌鸦、三个白兵
 */
@Injectable()
export class ThreeCandlePatterns {
  ...
  detect(candles: Candle[]): PatternResult[] {
    ...
    // 优先检测十字形态（更精确的形态）
    const dojiMorningStar = this.detectDojiMorningStar(twoDaysAgo, previous, current);
    if (dojiMorningStar) {
      patterns.push(dojiMorningStar);
      return patterns; // 如果检测到十字启明星，不再检测普通启明星
    }

    const dojiEveningStar = this.detectDojiEveningStar(twoDaysAgo, previous, current);
    if (dojiEveningStar) {
      patterns.push(dojiEveningStar);
      return patterns; // 如果检测到十字黄昏之星，不再检测普通黄昏之星
    }
  ...
  /**
   * 十字启明星 (Doji Morning Star) - 更强的看涨信号
   * 特征：
   * 1. 第一日是长阴线（下跌趋势延续）
   * 2. 第二日是十字星（开盘价和收盘价非常接近，表示市场犹豫不决）
   * 3. 第三日是长阳线（上涨反转）
   */
  private detectDojiMorningStar(first: Candle, second: Candle, third: Candle): PatternResult | null {
    ...
    const secondTotalRange = second.high - second.low;
    ...
    // 规则2：第二日必须是真正的十字星（实体不超过总范围的10%）
    if (secondTotalRange === 0 || secondBodySize > secondTotalRange * 0.1) {
      return null;
    }
    ...
    // 计算置信度（十字形态比普通形态置信度更高）
    let confidence = 0.8;
    if (hasGap) confidence += 0.1;
    if (thirdBodySize > firstBodySize) confidence += 0.1;

    return {
      patternType: 'doji_morning_star',
      patternName: '十字启明星',
      confidence: Math.min(1, confidence),
      signal: 'buy',
      description: '阴线-十字星-阳线的组合，底部强反转信号'
    };
  }

  /**
   * 十字黄昏之星 (Doji Evening Star) - 更强的看跌信号
   * 特征：
   * 1. 第一日是长阳线（上涨趋势延续）
   * 2. 第二日是十字星（开盘价和收盘价非常接近，表示市场犹豫不决）
   * 3. 第三日是长阴线（下跌反转）
   */
  private detectDojiEveningStar(first: Candle, second: Candle, third: Candle): PatternResult | null {
    const firstBodySize = Math.abs(first.close - first.open);
    const secondBodySize = Math.abs(second.close - second.open);
    const thirdBodySize = Math.abs(third.close - third.open);
    const secondTotalRange = second.high - second.low;
    ...
    return {
      patternType: 'doji_evening_star',
      patternName: '十字黄昏之星',
      confidence: Math.min(1, confidence),
      signal: 'sell',
      description: '阳线-十字星-阴线的组合，顶部强反转信号'
    };
  }
