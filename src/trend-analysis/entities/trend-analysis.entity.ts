...
export class TrendAnalysis {
  ...
  @Column({ type: 'decimal', precision: 10, scale: 4, comment: '涨幅%（相对上一交易日）' })
  changePercent: number;

  @Column({ type: 'decimal', precision: 10, scale: 4, nullable: true, comment: '偏离率（现价相对MA20）' })
  deviationRate: number | null;

  @Column({ type: 'decimal', precision: 10, scale: 2, nullable: true, comment: '量比' })
  volumeRatio: number | null;

  @Column({ length: 10, comment: '趋势状态: above(高于MA20)/below(低于MA20)' })
  trendStatus: 'above' | 'below';

  @Column({ type: 'date', nullable: true, comment: '状态转变日期（现价超过或低于MA20的日期）' })
  statusChangeDate: Date | null;

  @Column({ type: 'decimal', precision: 10, scale: 4, nullable: true, comment: '区间涨幅%（从状态转变日到当前）' })
